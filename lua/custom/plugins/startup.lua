-- local startup_screen_opened = false
...
local PLUGIN_NAME = "startup_screen"
...
local function draw_startup_screen(buf, logo_width, logo_height)
...
local function create_and_set_startup_screen_buf(default_buff)
	...
	vim.api.nvim_set_option_value("filetype", "startup_screen", { buf = intro_buff })
...
local function redraw()
	unlock_buf(startup_screen_buff)
	vim.api.nvim_buf_set_lines(startup_screen_buff, 0, -1, true, {})
	lock_buf(startup_screen_buff)
	draw_startup_screen(startup_screen_buff, INTRO_LOGO_WIDTH, INTRO_LOGO_HEIGHT)
end

local function display_startup_screen(payload)
	...
	startup_screen_buff = create_and_set_startup_screen_buf(default_buff)
	set_options()
...
local function setup(options)
	...
	vim.api.nvim_create_autocmd("VimEnter", {
		group = autocmd_group,
		callback = display_startup_screen,
		once = true
	})
end
